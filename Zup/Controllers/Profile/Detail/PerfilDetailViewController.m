...
- (void)confirmFlagReport {
    ...
    loading.frame = CGRectMake((alert.view.frame.size.width - 64) / 2, 0, 64, 64);
...
- (void) didReceiveOffensive:(NSData*)data {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self.reportLoading dismissViewControllerAnimated:YES completion:nil];
        
        [self popView];
        
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Reportar Relato"
                                                                       message:message
                                                                preferredStyle:UIAlertControllerStyleAlert];
...
- (void) didReceiveOffensiveError:(NSError*)error operation:(TIRequestOperation*)operation data:(NSData*)data {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (data == nil) {
            ...
            [Utilities alertWithError:message inViewController:self];
        }
    });
}
...
- (void)flagReport {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Deseja reportar esse relato como conteúdo ofensivo ou inapropriado?"
                                                                   message:nil
                                                            preferredStyle:UIAlertControllerStyleActionSheet];
    [alert addAction:[UIAlertAction actionWithTitle:@"Cancelar" style:UIAlertActionStyleCancel handler:nil]];
    [alert addAction:[UIAlertAction actionWithTitle:@"Reportar" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        dispatch_async(dispatch_get_main_queue(), ^{
